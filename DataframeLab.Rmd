...
```{r}
is_vector <- is.vector(grants$organization)
...
```{r}
total_amount <- mean(grants$total_amount, na.rm = TRUE)
...
organization_smallest_grant <- grants$organization[smallest_grant_index]
...
```{r}
grants_2010 <- grants[grants$total_amount == 2010, ]
...
```{r}
grants_2010 <- grants[format(as.Date(grants$total_amount), "%Y") == "2010", ]

total_awarded_2010 <- sum(grants_2010$total_amount, na.rm = TRUE)

print(total_awarded_2010)
...
```{r}
USPersonalExpenditure_long <- USPersonalExpenditure_df %>%
  pivot_longer(
    cols = -Category,
    names_to = "Year",
...
```{r}
personal_care_1940 <- USPersonalExpenditure_long %>%
  filter(Category == "Personal Care" & Year == "1940")

personal_care_1940$Expenditure
...
```{r}
food_tobacco_1960 <- USPersonalExpenditure_long %>%
  filter(Category == "Food and Tobacco" & Year == "1960")

food_tobacco_1960$Expenditure
...
expenditure_data <- data.frame(
  Category = c("Housing", "Food", "Transportation", "Healthcare", "Education"),
  Amount = c(3000, 2000, 1500, 1000, 500) # Example amounts
)

highest_expenditure <- expenditure_data %>%
  arrange(desc(Amount)) %>%
  slice(1)
...
```{r}
expenditure_data <- data.frame(
  Year = c(1960, 1960, 1960, 1970, 1970, 1970),
  Category = c("Housing", "Food", "Transportation", "Healthcare", "Education", "Food"),
  Amount = c(3000, 2000, 1500, 4000, 1000, 500) 
)
...
```{r}
lowest_categories_by_year <- sapply(unique_years, lowest_category)
